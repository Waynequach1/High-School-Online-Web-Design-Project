...
<HTML>
	...
	<BODY style="background-color: 50B2DF;"> 
		...
		<%
			...
			User = Session("ScoreChange")
			Score = Request.Form("txtScoreChange")
			
			if User = "" then
				Session("ErrorMsg") = "No request for changing scores received. Redirected back to main page."
				Response.Redirect("http://cozmathematics.1apps.com/main.asp")
			end if
			...
			do while not objRS.EOF
				...
				if UCase(User) = AUser then
					Response.Write("The user: " & User & " has had their score changed from " & objRS("Rank") & " to " & Score)
					Response.Write("<br>")
					Response.Write("The user's score has now been changed. The leaderbaords have been updated but the user's session has not. The session will update the next time the user logs in.")
					objRS("Rank") = Score
					ObjRs.Update
					exit do
				end if
		...
		<br>
		<FONT SIZE=5>
		<br>To visit the leaderboards page click <a href="leaderboards.asp">Here</a>
		</FONT>
		<HR>
